	...
	testCases := []struct {
	...
	}{
		{
			...
			testCase: func(t *testing.T) {
				...
				var as *Animal
				as, err = query.Find(db.Animal, Animal{Id: a.Id})
				if err != nil {
				...
				err = query.Remove(db.Animal, Animal{Id: as.Id})
				if err != nil {
				...
				_, err = query.Find(db.Animal, Animal{Id: as.Id})
		...
		},
		{
			...
			testCase: func(t *testing.T) {
				...
				animals = nil
				animals, err = query.Select(db.Animal).From(db.Animal).Where(wh.Like(&db.Animal.Name, "%Cat%")).RowsAsSlice()
				if err != nil {
					t.Fatalf("Expected a select, got error: %v", err)
				}
				...
				err = query.Delete(db.Animal).Where(wh.Like(&db.Animal.Name, "%Cat%"))
				if err != nil {
		...
		},
		{
			...
			testCase: func(t *testing.T) {
				err = query.Delete(db.DB).Where(wh.Equals(&db.Animal.Id, 1))
		...
		},
		{
			...
			testCase: func(t *testing.T) {
				b := 2
				err = query.Delete(db.Animal).Where(wh.Equals(&b, b))
		...
		},
		{
			...
			testCase: func(t *testing.T) {
				...
				cancel()
				err = query.DeleteContext(ctx, db.Animal).Where()
